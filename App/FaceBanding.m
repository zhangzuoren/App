...
@interface FaceBanding ()<AVCaptureMetadataOutputObjectsDelegate,UIAlertViewDelegate>
{
    BOOL _loading;//校验照片中
    int _count;
}
...
-(UIImageView *)banner{
    if(!_banner){
        _banner=[[UIImageView alloc] initWithFrame:CGRectMake(0, ([UIScreen mainScreen].bounds.size.height-[UIScreen mainScreen].bounds.size.width)/2.0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.width)];
        _banner.image=[UIImage imageNamed:@"face_banner"];
        _banner.alpha=0.4;
    }
    return _banner;
}
-(UIView *)top{
    if(!_top){
        _top=[[UIView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, ([UIScreen mainScreen].bounds.size.height-[UIScreen mainScreen].bounds.size.width)/2.0)];
...
-(UIView *)bottom{
    if(!_bottom){
        _bottom=[[UIImageView alloc] initWithFrame:CGRectMake(0, ([UIScreen mainScreen].bounds.size.height-[UIScreen mainScreen].bounds.size.width)/2.0+[UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.width, ([UIScreen mainScreen].bounds.size.height-[UIScreen mainScreen].bounds.size.width)/2.0)];
...
-(BaseIconLabel *)noFace{
    if(!_noFace){
        ...
        _noFace.center=CGPointMake([UIScreen mainScreen].bounds.size.width/2, STATUS_BAR_HEIGHT+22);
...
-(BaseIconLabel *)haveFace{
    if(!_haveFace){
        ...
        [_haveFace sizeToFitWithText:@"正在绑定中，请保持当前姿势"];
        _haveFace.center=CGPointMake([UIScreen mainScreen].bounds.size.width/2, STATUS_BAR_HEIGHT+22);
    ...
    if (metadataObjects.count > 0) {
        ...
        if(_count>72 && !_loading){
            ...
            [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:stillConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
                ...
                [Http postImagesData:data uploadUrl:[NSString stringWithFormat:@"%@/qdweb/wjkUpload.jsp",BaseUrl] name:@"wjkfile" parametersDic:@{@"sfz":[[UserInfo sharedUserInfo] idnum]} uploadProgress:nil success:^(id requestObj) {
                    [self dismissViewControllerAnimated:NO completion:nil];
                    if(self.block){
                        self.block();
                    }
